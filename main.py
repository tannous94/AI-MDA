...
def plot_distance_and_expanded_wrt_weight_figure(
    ...
    p1, = ax1.plot(weights, total_cost, label='Solution cost', color='b')  # TODO: pass the relevant params instead of `...`.
    ...
    p2, = ax2.plot(weights, total_nr_expanded, label='#Expanded states', color='r')  # TODO: pass the relevant params instead of `...`.
...
def run_astar_for_weights_in_range(heuristic_type: HeuristicFunctionType, problem: GraphProblem, n: int = 30,
    ...
    nr_states_expanded = []
    weight_used = []
    weights_arrays = np.linspace(low_heuristic_weight, high_heuristic_weight, n)
    for w in weights_arrays:
        wastar = AStar(heuristic_type, w, max_nr_states_to_expand)
        res = wastar.solve_problem(problem)
        if res.is_solution_found:
            solution_cost.append(res.solution_g_cost)
            nr_states_expanded.append(res.nr_expanded_states)
            weight_used.append(w)
    plot_distance_and_expanded_wrt_weight_figure(problem.name, weight_used, solution_cost, nr_states_expanded)
...
def toy_map_problem_experiments():
    ...
    res1 = astar_null.solve_problem(toy_map_problem)
    print(res1)
    ...
    res2 = astar_air_dist.solve_problem(toy_map_problem)
    print(res2)
...
def mda_problem_with_astar_experiments():
    ...
    mda_astar = AStar(MDAMaxAirDistHeuristic)
    res = mda_astar.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res)

    # Ex.25
    # TODO: create an instance of `AStar` with the `MDASumAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    mda_astar_sum = AStar(MDASumAirDistHeuristic)
    res1 = mda_astar_sum.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res1)

    # Ex.28
    # TODO: create an instance of `AStar` with the `MDAMSTAirDistHeuristic`,
    #       solve the `moderate_mda_problem_with_distance_cost` with it and print the results.
    mda_astar_mst = AStar(MDAMSTAirDistHeuristic)
    res2 = mda_astar_mst.solve_problem(moderate_mda_problem_with_distance_cost)
    print(res2)
...
def monetary_cost_objectives_mda_problem_experiments():
    ...
    uc_small = UniformCost()
    res = uc_small.solve_problem(small_mda_problem_with_monetary_cost)
    print(res)

    # Ex.32
    # TODO: create an instance of `UniformCost`
    #       solve the `moderate_mda_problem_with_monetary_cost` with it and print the results.
    uc_moderate = UniformCost()
    res1 = uc_moderate.solve_problem(moderate_mda_problem_with_monetary_cost)
    print(res1)
...
def multiple_objectives_mda_problem_experiments():
    ...
    mda_astar = AStar(MDATestsTravelDistToNearestLabHeuristic)
    res = mda_astar.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res)
    ...
    #          elsewhere next.
    #       Solve the `moderate_mda_problem_with_tests_travel_dist_cost` with it and print the results.
    astar_ins = AStar(MDAMSTAirDistHeuristic)
    optimal_distance_cost = astar_ins.solve_problem(moderate_mda_problem_with_distance_cost).solution_g_cost
    max_distance_cost = (1 + 0.6) * optimal_distance_cost
    astar_ins_2 = AStar(MDATestsTravelDistToNearestLabHeuristic, open_criterion= lambda node: node.cost.distance_cost <= max_distance_cost)
    res1 = astar_ins_2.solve_problem(moderate_mda_problem_with_tests_travel_dist_cost)
    print(res1)
...
def mda_problem_with_astar_epsilon_experiments():
    ...
    astar_eps = AStarEpsilon(MDAMSTAirDistHeuristic, within_focal_priority_function=within_focal_h_sum_priority_function, focal_epsilon=0.23, max_focal_size=40)
    res1 = astar_eps.solve_problem(small_mda_problem_with_distance_cost)
    print(res1)
