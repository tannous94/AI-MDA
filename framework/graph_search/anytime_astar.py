    ...
    def solve_problem(self, problem: GraphProblem) -> SearchResult:
        with Timer(print_title=False) as timer:
            ...
            while (high_heuristic_weight - low_heuristic_weight) > 0.01:
                ...
                astar = AStar(heuristic_function_type= self.heuristic_function_type, heuristic_weight= mid, max_nr_states_to_expand= self.max_nr_states_to_expand_per_iteration)
                ...
                if (res.solution_g_cost is not None) and best_solution.solution_g_cost > res.solution_g_cost:
                    best_solution = res
