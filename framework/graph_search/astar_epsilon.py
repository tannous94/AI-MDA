...
class AStarEpsilon(AStar):
    ...
    def _extract_next_search_node_to_expand(self, problem: GraphProblem) -> Optional[SearchNode]:
        ...
        item = self.open.peek_next_node()
        f_max = item.expanding_priority * (1 + self.focal_epsilon)

        focal = []
        focal_values = []

        while item.expanding_priority <= f_max:
            if (self.max_focal_size is not None) and len(focal) >= self.max_focal_size:
                break
            focal.append(item)
            focal_values.append(self.within_focal_priority_function(item, problem, self))
            self.open.pop_next_node()
            if self.open.is_empty():
                break
            item = self.open.peek_next_node()

        opt = focal.pop(np.argmin(focal_values))
        for elem in focal:
            self.open.push_node(elem)
        self.close.add_node(opt)
        return opt
