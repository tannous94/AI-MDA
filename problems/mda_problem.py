...
class MDAState(GraphProblemState):
    ...
    def get_total_nr_tests_taken_and_stored_on_ambulance(self) -> int:
        ...
        """
        return sum(x.nr_roommates for x in self.tests_on_ambulance)
...
class MDAProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        for apartment in self.get_reported_apartments_waiting_to_visit(state_to_expand):
            if apartment.nr_roommates <= state_to_expand.nr_matoshim_on_ambulance and \
            apartment.nr_roommates <= (self.problem_input.ambulance.total_fridges_capacity - state_to_expand.get_total_nr_tests_taken_and_stored_on_ambulance()):
                new_taken = state_to_expand.tests_on_ambulance.union(frozenset([apartment]))
                new_matoshim = state_to_expand.nr_matoshim_on_ambulance - apartment.nr_roommates
                next_succ = MDAState(apartment, new_taken, state_to_expand.tests_transferred_to_lab, new_matoshim, state_to_expand.visited_labs)
                cost = self.get_operator_cost(state_to_expand, next_succ)
                yield OperatorResult(next_succ, cost, 'visit ' + apartment.reporter_name)
        ...
        for lab in self.problem_input.laboratories:
            ...
            if (state_to_expand.get_total_nr_tests_taken_and_stored_on_ambulance()) > 0 or (lab not in state_to_expand.visited_labs):
                if lab not in state_to_expand.visited_labs:
                    ind = 1
                new_taken = frozenset()
                new_trans = state_to_expand.tests_transferred_to_lab.union(state_to_expand.tests_on_ambulance)
                new_matoshim = state_to_expand.nr_matoshim_on_ambulance + lab.max_nr_matoshim * ind
                new_visited_labs = state_to_expand.visited_labs.union(frozenset([lab]))
                next_succ = MDAState(lab, new_taken, new_trans, new_matoshim, new_visited_labs)
                cost = self.get_operator_cost(state_to_expand, next_succ)
                yield OperatorResult(next_succ, cost, 'go to lab ' + lab.name)
    ...
    def get_operator_cost(self, prev_state: MDAState, succ_state: MDAState) -> MDACost:
        ...
        cost_dist = float('inf')
        cost_mont = float('inf')
        cost_tests_tr = float('inf')

        cost = self.map_distance_finder.get_map_cost_between(prev_state.current_location, succ_state.current_location)
        if cost is not None:
            active_fridges = math.ceil(float(prev_state.get_total_nr_tests_taken_and_stored_on_ambulance()) / float(self.problem_input.ambulance.fridge_capacity))
            fridge_gas_cons = sum(self.problem_input.ambulance.fridges_gas_consumption_liter_per_meter[:active_fridges])

            cost_dist = cost

            cost_mont = self.problem_input.gas_liter_price * (self.problem_input.ambulance.drive_gas_consumption_liter_per_meter + fridge_gas_cons) * cost

            if succ_state.current_site in self.problem_input.laboratories:
                if prev_state.get_total_nr_tests_taken_and_stored_on_ambulance() > 0:
                    cost_mont += succ_state.current_site.tests_transfer_cost
                if succ_state.current_site in prev_state.visited_labs:
                    cost_mont += succ_state.current_site.revisit_extra_cost

            cost_tests_tr = prev_state.get_total_nr_tests_taken_and_stored_on_ambulance() * cost
        return MDACost(cost_dist, cost_mont, cost_tests_tr, self.optimization_objective)
    ...
    def is_goal(self, state: GraphProblemState) -> bool:
        ...
        return set(self.problem_input.reported_apartments) == set(state.tests_transferred_to_lab)
    ...
    def get_reported_apartments_waiting_to_visit(self, state: MDAState) -> List[ApartmentWithSymptomsReport]:
        ...
        """
        return sorted(list(set(self.problem_input.reported_apartments).difference(state.tests_on_ambulance.union(state.tests_transferred_to_lab))), key=lambda elem: elem.report_id)
    ...
    def get_all_certain_junctions_in_remaining_ambulance_path(self, state: MDAState) -> List[Junction]:
        ...
        """
        remaining_routes = [state.current_location]
        for apartment in self.get_reported_apartments_waiting_to_visit(state):
            remaining_routes.append(apartment.location)
        return sorted(remaining_routes, key=lambda item: item.index)
