...
class MDASumAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        distance_of_remaining_route = 0.0
        next_junction = state.current_location
        curr_junction = state.current_location
        all_certain_junctions_in_remaining_ambulance_path.remove(next_junction)

        while len(all_certain_junctions_in_remaining_ambulance_path) is not 0:
            (curr_min, index_min) = (float('inf'), next_junction.index)
            for junction in all_certain_junctions_in_remaining_ambulance_path:
                curr = self.cached_air_distance_calculator.get_air_distance_between_junctions(next_junction, junction)
                if (curr, junction.index) < (curr_min, index_min):
                    (curr_min, index_min) = (curr, junction.index)
                    curr_junction = junction
            next_junction = curr_junction
            distance_of_remaining_route += curr_min
            all_certain_junctions_in_remaining_ambulance_path.remove(next_junction)

        return distance_of_remaining_route
...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
        ...
        for junction in junctions:
            junction_graph.add_node(junction)

        for j1 in junctions:
            for j2 in junctions:
                if not j1 == j2:
                    edge_weight = self.cached_air_distance_calculator.get_air_distance_between_junctions(j1, j2)
                    junction_graph.add_weighted_edges_from([(j1, j2, edge_weight)])

        mst = nx.minimum_spanning_tree(junction_graph, weight='weight')
...
class MDATestsTravelDistToNearestLabHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        def air_dist_to_closest_lab(junction: Junction) -> float:
            ...
            """
            return min([junction.calc_air_distance_from(lab.location) for lab in labs])

        remained_cost = air_dist_to_closest_lab(state.current_location)
        if state.get_total_nr_tests_taken_and_stored_on_ambulance() > 0:
            remained_cost = remained_cost * state.get_total_nr_tests_taken_and_stored_on_ambulance()

        return remained_cost + sum([air_dist_to_closest_lab(apartment.location) * apartment.nr_roommates for apartment in self.problem.get_reported_apartments_waiting_to_visit(state)])
